...
def count_active_params(state_dict):
    '''
    Count non-zero parameters in the model's state dictionary.
    This is an approximate measurement of size in
    terms of non-masked parameters.
    '''
    total = 0
    for i in state_dict:
...
def test_only(model, mask, mask_sizing, evalloader):
    '''
    Test and report accuracy and size. Apply mask according to the guide
    given by the mask sizing dict, and evaluate it on the evalloader.
    '''
    model.eval()
...
def test_without_size(model, evalloader):
    '''
    Test and report loss only. Useful for evolution.
    '''
    model.eval()
    avg_loss = 0
    ...
    avg_loss = avg_loss/len(evalloader)
    return avg_loss

def test_acc(model, evalloader):
    '''
    Test and report accuracy only. Useful for final assessment of evolved
    populations.
    '''
    model.eval()
    avg_acc = 0
...
def finetune(model, mask, mask_sizing, extloader, evalloader, epochs):
    '''
    Train and report best achieved accuracy (tested after every epoch) as well
    as size.
    '''
...
def test_offspring(model, mask, mask_sizing, extloader, epochs):
    '''
    Test offspring during evolution, report accuracy and size.
    '''
...
def initialize_and_evaluate_parents(progenitor_arr, model_arr, data_arr, dims, pop, subpop_size, mask_sizing, pretrained):
    '''
    Initialize two task populations and test, returning accuracy and size.
    '''
    population = []
    counter = 0
    counter2 = 0.005
...
def evaluate_offspring(model_arr, train_data_arr, pop, mask_sizing, offspring):
    '''
    Apply masking to offspring and test.
    '''
    results = {}
    for i in range(pop):
...
def final_finetune(model_arr, fitnesses, train_data_arr, test_data_arr, pop, mask_sizing, offspring, subpop_size, finetune_epochs):
    '''
    Final training post evolution. First, select only pareto optimal
    members of each population. Then train each population only on the
...
def get_final_population(list_of_pops, pop_sizes):
    '''
    Get final population after evoltion.
    '''
    final_pop = []
...
def multitaskMFEA(target_dir,
    ...
    # Initialize JATs (Generalists) and evaluate.
    ...
    for generation in range(gen):  # Main evolutionary loop
        ...
        parent = []
        '''
        Select the initial parent population.
        '''
        for i in range(pop):
            temp = Chromosome()
        ...
        '''
        Assign parent masks to specialized subpopulations and assign
        skill factors.
        '''
        ...
        for i in range(0, pop, 2):
            ...
            child2 = Chromosome()
            '''
            Cross over and mutate according to the MFEA algorithm. 
            '''
            if MFEA_II:
