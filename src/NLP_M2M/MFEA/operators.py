...
def learn_models(subpops):
  ...
  for k in range(K):
    ...
    models.append(Crundle(mean, std, num_sample))
  return models
...
def learn_rmp(subpops, D):
  ...
  for k in range(K - 1):
    for j in range(k + 1, K):
      ...
      random_addition = np.random.uniform() * 0.1
      rmp += random_addition
      rmp = np.clip(rmp, 0, 1)
      rmp_matrix[k, j] = rmp
      rmp_matrix[j, k] = rmp
...
#   return models
#
# def rmp_learning(k, j, rmp_matrix, probmatrix, K):
#     rmp = fminbound(lambda rmp: log_likelihood(rmp, probmatrix, K), 0, 1)
#     rmp += np.random.randn()* 0.001
#     rmp = np.clip(rmp, 0, 1)
#     rmp_matrix[k, j] = rmp
#     rmp_matrix[j, k] = rmp
#
...
#         probmatrix[1][:, 0] = models[k].density(subpops[j])
#         probmatrix[1][:, 1] = models[j].density(subpops[j])
#         threads.append(Thread(target=rmp_learning, args=(k, j, rmp_matrix, probmatrix, K)))
#   for t in threads:
#       t.start()
#   for t in threads:
#       t.join()
